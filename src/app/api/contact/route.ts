...
export async function POST(req: NextRequest) {
  const { company, phone, units, consult } = await req.json();

  if (!company || !phone) {
    return NextResponse.json({ error: "회사명과 연락처는 필수입니다." }, { status: 400 });
  }
    ...
    <div style="font-family:'Apple SD Gothic Neo',sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#fff;border:1px solid #E4E6E8;border-radius:12px;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          ...
          <td style="padding:12px 0;border-bottom:1px solid #F2F3F4;font-size:15px;color:#161616;">${company}</td>
        </tr>
        <tr>
          ...
          <td style="padding:12px 0;border-bottom:1px solid #F2F3F4;font-size:15px;color:#161616;">${phone}</td>
        </tr>
        <tr>
          ...
          <td style="padding:12px 0;border-bottom:1px solid #F2F3F4;font-size:15px;color:#161616;">${units || "미입력"}</td>
        </tr>
        <tr>
          <td style="padding:12px 0;font-size:14px;font-weight:700;color:#71767D;vertical-align:top;">상담 방식</td>
          <td style="padding:12px 0;font-size:15px;color:#161616;">${consult}</td>
        </tr>
      </table>
    </div>
  `;
